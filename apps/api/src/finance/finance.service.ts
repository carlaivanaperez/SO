...
export class FinanceService {
  ...
  async getConfig(): Promise<FinanceConfig> {
    const config = await this.prisma.financeConfig.upsert({
      where: { id: 1 },
      create: { id: 1, lateFeeDailyPercent: DEFAULT_LATE_FEE },
      update: {},
    });
  ...
  async updateConfig(dto: UpdateFinanceConfigInput): Promise<FinanceConfig> {
    await this.prisma.$transaction([
      this.prisma.financeConfig.upsert({
        where: { id: 1 },
        create: { id: 1, lateFeeDailyPercent: new Prisma.Decimal(dto.lateFeeDailyPercent) },
        update: { lateFeeDailyPercent: new Prisma.Decimal(dto.lateFeeDailyPercent) },
      }),
